...
- In project scope, a local directory outside the project root is recorded by its absolute path, which other machines cannot resolve. `add` warns and asks before it records it. `-y` answers yes. Use `--copy` to put the files in the project, or `-g` to install for this machine only.
