...
When a detected agent reads two or more of the chosen directories, `niksi` warns that the agent loads skills twice and names the directories to drop. It stays quiet when every chosen directory is the only one some detected agent reads.
