...
| field | meaning |
| --- | --- |
| `source` | Git URL of the source, or `local:` followed by its path. A directory inside the project root is recorded relative to it. One outside is recorded by its absolute path, which other machines cannot resolve, so `add` asks first. |
